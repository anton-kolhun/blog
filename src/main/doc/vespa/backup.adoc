...
We run a dedicated `vespa-backup` `StatefulSet` (single pod) inside the same Kubernetes cluster as Vespa - see the https://github.com/anton-kolhun/vespa-sample/tree/main/vespa-backup/vespa-helm[Helm chart] for the full Kubernetes resources. It shares the same network, which means it can reach Vespa's internal `configserver` and content cluster directly.
